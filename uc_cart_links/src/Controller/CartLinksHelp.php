...
//  t('Specify the redirection by adding ?destination=url where url is the page to go to.'),
    ...
    $rows = array(
      ...
      array('m', t('Displays a message to the customer when the link is clicked.'), t('A <a href="@url">numeric message ID</a> to identify which message to display.', ['@url' => Url::fromRoute('uc_cart_links.settings')])),
    ...
    $rows = array(
      ...
      (The add-to-cart message may be enabled on the <a href="@url">cart settings page</a>).', ['@url' => Url::fromRoute('uc_cart.cart_settings')]), t('None.')),
    );
    $build['args'] = array(
    ...
    $build['help'] = array(
      '#prefix' => '<p>',
      '#markup' => t('<a href=":url">Visit the settings page</a> to set preferences, define messages, and restrict links that may be used.', [':url' => Url::fromRoute('uc_cart_links.settings')]),
      '#suffix' => '</p>',
    );
